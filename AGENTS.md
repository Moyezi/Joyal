...
- `dio` 请求 Subsonic API；`cached_network_image` + `flutter_cache_manager` 管理封面缓存，UI 通过 `CachedDiskImage` 先查磁盘命中再走网络；`flutter_secure_storage` 保存凭据、搜索历史、播放器进度和缓存上限，缓存上限另写非敏感 JSON 兜底。
...
- 歌词、下载与缓存：`lib/services/lyrics_service.dart`、`lib/screens/lyrics_screen.dart`、`lib/services/app_cache_service.dart`、`lib/widgets/cached_disk_image.dart`、`lib/services/download_service.dart`、`lib/screens/download_manager_screen.dart`、`lib/services/cache_stats_service.dart`、`lib/providers/cache_provider.dart`、`lib/screens/cache_management_screen.dart`。
...
- 歌词缓存键按 `baseUrl + username + song.id` 的哈希作用域生成；空歌词也要短期缓存，避免无歌词歌曲每次进入歌词页都请求服务器。
- `LyricsScreen` 的内存 Future 缓存必须按服务器/用户/歌曲区分，失败后移除，避免一次失败后同一歌曲一直卡失败状态。
- 播放页横滑/点击进入歌词页时，`LyricsScreen` 一旦初始化就应立即加载内容；不要用“动画完成后才 loadContent”的占位分支，否则重进应用首次滑出歌词页会先显示歌词占位图标，完全拖出后才加载。
...
- 图片显示优先使用 `lib/widgets/cached_disk_image.dart` 的 `CachedDiskImage`：先 `DefaultCacheManager().getFileFromCache(cacheKey)`，命中则直接 `Image.file`，未命中再走 `CachedNetworkImage`。磁盘查询期间有稳定 URL 时不要回退到默认占位；可用透明/空盒或已加载状态避免首帧闪占位。不要直接在新封面/头像 UI 里使用 `CachedNetworkImage`，否则磁盘已缓存时仍会先显示默认占位。
- Subsonic 封面 URL 每次会因随机 salt/token 变化；专辑/歌曲封面 `cacheKey` 应使用稳定 `coverArtId`，歌手头像不要用 `String.hashCode` 做持久缓存 key，应使用稳定哈希或可用的 `coverArtId`。
- `AppCacheService` 管理小型 JSON 缓存。不要在实例方法里用 `Isolate.run(() => jsonEncode(value))` 或捕获实例字段的闭包，曾导致 `_writeTail` 这类不可发送对象被捕获，JSON 缓存写入失败。
- 自动清理 Slider 档位：500MB / 1GB / 2GB / 5GB / 无限制；上限写入 `flutter_secure_storage`，并同步写入 `AppCacheService` 的 `cache_settings` JSON 兜底，超出后按文件修改时间 LRU 删除最旧临时音频。
...
- 已有基础 Widget/逻辑测试，含 GlassTopBar 搜索图标、首页动画、主页侧边栏设置入口、收藏页无“我的”按钮、侧边栏右滑不纵向滚动、波形几何、封面取色基础、播放页视觉歌曲选择和歌词空结果缓存。
