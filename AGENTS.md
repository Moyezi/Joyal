...
- 图片显示优先使用 `lib/widgets/cached_disk_image.dart` 的 `CachedDiskImage`：先 `DefaultCacheManager().getFileFromCache(cacheKey)`，命中则直接 `Image.file`，未命中再走 `CachedNetworkImage`。不要直接在新封面/头像 UI 里使用 `CachedNetworkImage`，否则磁盘已缓存时仍会先显示默认占位。
