...
核心技术：Flutter / Dart / Material 3、Riverpod、`just_audio`、`dio`、本地磁盘封面缓存。安全底线：真实凭据和 DeepSeek API Key 只能进 secure storage；Android 媒体桥不得传流媒体 URL、token、密码或 `baseUrl`；分类请求只发歌曲文字元数据。
...
- `.agents/skills/joyal-classification/SKILL.md`：智能分类、DeepSeek 安全、固定词表、JSON 存储、手动修正、分类入口。
