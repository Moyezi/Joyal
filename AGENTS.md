...
- `just_audio` 管 HTTP/本地音频和真实多曲目队列；`dio` 请求 Subsonic API；`flutter_cache_manager` 管封面缓存。
- 凭据、搜索历史、播放器进度、主题和缓存上限写入 `flutter_secure_storage`；缓存上限另写 `AppCacheService` 非敏感 JSON 兜底。
...
- 根页面用 `Stack`；页面铺底，`MiniPlayer` 与 `AppBottomNav` 组成透明 Dock 覆盖底部。有当前歌曲时显示迷你播放栏，无歌曲时隐藏；列表底部内边距动态避让 Dock。
- 独立详情页的返回按钮固定在页面级左上安全区；复用内容组件（如歌手内容、sheet 内容）不要自带返回栏或改变 TabBar/标题区域高度。
- 歌曲列表行优先复用 `SongTile` + `SongActionsSheet`，保持曲库、收藏、专辑、歌手等入口的播放态、下载标记、更多菜单和排版一致。
- 首页右滑打开 `HomeSidebar`，侧边栏约占屏幕 70%，右侧保留主页预览；主页内容、MiniPlayer 和 Dock 随进度右移、轻微缩小并模糊。
- 侧边栏手势由 `_MainShellState` 驱动，要参与 Flutter 手势竞争，避免右滑时首页纵向滚动。“最近添加”横向列表是侧边栏手势排除区，由 `HomeScreen.onExclusionZoneChanged` 上报。
...
- Toast 统一用 `lib/utils/app_toast.dart` 的 `showAppToast(...)`，不要直接散落 `ScaffoldMessenger.showSnackBar(...)`。Toast 参考 Android 原生底部偏上位置，宽度随文字自适应，深浅色分别适配，无阴影，使用淡入淡出。
- 封面取色由 `AlbumVisualPalette` 处理，缓存键含 brightness；`DynamicAlbumBackground` 回退色用 `AlbumVisualPalette.fallbackFor(brightness)`，并尽量使用稳定 `coverArtId`，避免认证 URL 刷新导致重复取色。
...
- 测试：`flutter test`。常用单测：`flutter test test/widget_test.dart`、`flutter test test/now_playing_visual_song_test.dart`、`flutter test test/cache_provider_test.dart`。
