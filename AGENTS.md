...
- 歌词页是沉浸式页面，不显示返回键和标题“歌词”；顶部只固定显示当前歌曲名和歌手，不随歌词列表滚动。歌词区毛玻璃/深度模糊效果要作为同一层延伸到状态栏和标题背后，标题文字和歌手浮在其上保持清晰，不要单独做割裂的状态栏/标题栏玻璃。歌词列表的可绘制区域从标题/歌手栏下方开始，歌词不应从标题栏背后滚过。
...
- 迷你播放栏歌词滚动动画使用独立非线性曲线控制位移、淡出和淡入；不要把所有元素退回同一线性进度或整块替换。
...
- 迷你播放栏圆形封面可旋转且要跨 Hero/重建保存旋转相位；播放详情页大封面停留态必须静止不旋转。进入/退出详情页的 Hero 飞行层可临时做角度过渡来对齐迷你栏当前相位，避免落点瞬移闪烁；共享逻辑在 `lib/widgets/now_playing_transition.dart`。
- 迷你栏与播放详情页的专辑封面 Hero 飞行层要同时负责形状和阴影过渡：打开时从圆形自然变为大圆角方形，阴影在压暗/铺底过程中提前渐入；关闭时形状要连续回到圆形并淡出阴影，不能在落地后才从四圆角或无阴影瞬间切到迷你栏样式。
