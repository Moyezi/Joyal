...
- Keep `浮名`'s print stamp crisp, with no `MaskFilter.blur`; keyword stamps use the keyword's semantic color while other stamps use the configured stamp color, with peak alpha at 0.8 times the resolved color and fading with the stamp pulse. Do not paint a stamp for whitespace graphemes or Unicode punctuation. Reveal typed text with per-grapheme span colors instead of clipping one fully bright painter with glyph selection rectangles; tight line metrics can otherwise expose the tops of the next visual row. When the current line becomes passed, ease non-keyword glyphs from the active color into the passed gray over the existing camera transition while keyword colors remain stable.
