...
- Gate ordinary tokens' outward entrance ring with the cached DeepSeek climax timeline. AI semantic keywords keep their keyword-colored entrance ring even outside climax segments; non-keywords outside climax have zero ring intensity. Loading/error/no-key states may still use already-cached AI keyword rings while preserving the ordinary token reveal and soft halo.
...
- `浮名` uses a cached world-space three-column snake layout with sparse hero lines, allowing the article and camera path to expand left and right before moving into the next row. Block transitions and single-visual-line camera follow must remain continuous: interpolate between cached grapheme-box centers from fractional print progress instead of snapping on completed glyphs. When one lyric wraps into multiple visual rows, lock the camera's horizontal focus to the block center and retain only subtle vertical following so line wraps never cause left-right sweeps. Spread the vertical wrap move continuously across several surrounding graphemes for Chinese and across several surrounding Latin words for English; never compress it into one Chinese grapheme or Latin-letter interval. Keep English ink landing one letter at a time even though its camera rhythm is word-scaled. Its subtle paper tint is an edge-to-edge screen-space layer; never restore a finite world-space mask whose left/right/top/bottom edge can enter the camera. Lines overlapping a cached/recognized DeepSeek climax segment are prelaid out at about 116% of their normal floating-name size.
- During a genuinely long gap between the completed active line and the next non-empty timed line, `浮名` may reuse its existing frame controller for a few pixels of low-frequency camera translation and a sub-degree rotation. Keep short gaps still, ease the drift in only after the line settles, ease it out before the next line, and disable it whenever playback pauses, the stage is hidden/covered, or reduced motion is active; do not add another persistent ticker.
...
- Keep `浮名`'s print stamp crisp, with no `MaskFilter.blur`; keyword stamps use the keyword's semantic color while other stamps use the configured stamp color, with peak alpha at 0.8 times the resolved color and fading with the stamp pulse. Show the small line-end input-wait stamp only when the complete gap from the active line's print end to the next non-empty timed line is at least 3.6 seconds; begin its cadence immediately at print completion rather than 3.6 seconds later, clear it before the next line, and reuse the existing frame controller. The final lyric bypasses the 3.6-second threshold and starts the same cadence at print completion until playback stops. When the last stampable grapheme before the cursor is a semantic keyword, inherit that keyword color even through trailing punctuation; otherwise use the configured stamp color. Disable it while paused, hidden/covered, word-by-word-off, or reduced-motion. Do not paint a stamp for whitespace graphemes or Unicode punctuation. Reveal typed text with per-grapheme span colors instead of clipping one fully bright painter with glyph selection rectangles; tight line metrics can otherwise expose the tops of the next visual row. When the current line becomes passed, ease non-keyword glyphs from the active color into the passed gray over the existing camera transition while keyword colors remain stable.
- Let default scrolling lyrics and `浮名` share `lyrics/lyric_print_effect.dart` timing helpers: the default renderer may keep its blurred print stamp, while `浮名` uses a crisp 0.8-alpha stamp; both follow a baseline-to-10%-up-to-baseline bounce. Preserve the default renderer's color sweep/frontier glow, use no extra persistent controller, and keep motion off for whitespace, covered/disabled updates, and reduced-motion states.
- Resolve the shared AI lyric palette once in `LyricsScreen` and inject nullable base colors plus the theme-resolved keyword-color map into the default, `流光`, and `浮名` renderers. Renderers may use the UI-only `lyric_semantic_colors.dart` matcher, but they must not watch the AI palette provider or import its model, repository, protocol, or DeepSeek service. Keyword colors follow semantic/emotional meaning and must preserve readable contrast rather than becoming an arbitrary rainbow.
