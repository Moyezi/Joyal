...
- `浮名` uses a cached world-space article layout with sparse hero lines. Its vertical block transition and within-line horizontal camera follow must both remain continuous: interpolate between cached grapheme-box centers from fractional print progress instead of snapping on completed glyphs. Lines overlapping a cached/recognized DeepSeek climax segment are prelaid out at about 116% of their normal floating-name size.
