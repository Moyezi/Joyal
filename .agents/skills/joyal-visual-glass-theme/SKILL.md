...
- Resolve the shared AI lyric palette once in `LyricsScreen` and inject nullable Flutter `Color` values into the default, `流光`, and `浮名` renderers. Stage renderers own only their visual use of `primary`/`stamp`; they must not watch the AI palette provider or import its model, repository, protocol, or DeepSeek service.
