...
- Keep `浮名`'s print stamp crisp, with no `MaskFilter.blur`, and use the active glyph color with peak alpha at 0.8 times that color; fade the alpha with the stamp pulse. Do not paint a stamp for whitespace graphemes or Unicode punctuation. Reveal typed text with per-grapheme span colors instead of clipping one fully bright painter with glyph selection rectangles; tight line metrics can otherwise expose the tops of the next visual row.
