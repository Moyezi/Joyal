...
- Settled strongly blurred page/cover backgrounds may replace that live composition with a derived low-resolution PNG from `BlurredBackgroundCache`. Keep source-version, blur, viewport/DPR, fit, alignment, and content scale in the key; the offline render must use the live layer's exact crop/scale, without adding blur-radius-dependent overscan. Retain the live composition during generation, slider drags, and failures.
- Derived blur PNGs live under `libCachedImageData/derived_blurred_backgrounds`, are capped at 1536 px on the longest side, and prune to 24 entries / 32 MB. They therefore remain part of image-cache statistics, cleanup, and global LRU enforcement.
