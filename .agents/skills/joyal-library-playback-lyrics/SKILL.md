...
- “AI 文字配色”开关只显示在默认滚动和`流光`的歌词个性化“文字”栏，`浮名`不消费这套颜色。开启时按当前歌曲缓存或请求 DeepSeek；请求只含 title、album、artist。默认滚动已高亮字、前沿光晕和模糊打印印章使用 AI `primary`；`流光`的文字与高光光晕使用 `primary`，仅高潮时间轴覆盖的当前字或 Latin 单词外扩圆形光环使用 `stamp`。关闭、无有效缓存或请求失败时完整保留默认配色；不新增 ticker。
