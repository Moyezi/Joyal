...
- 三个效果让普通当前字或单词使用 AI `primary`，关键词实际出现的位置优先使用其语义色；下一字或单词开始后让已有文字在约 280 ms 内退回各渲染器原本的默认歌词色。无逐字时间时，默认滚动与`流光`可静态显示当前句关键词色。默认滚动的当前字前沿光晕和模糊打印印章跟随当前解析色；`流光`的当前文字与高光光晕跟随当前解析色，仅高潮时间轴覆盖的外扩圆环继续使用 `stamp`；`浮名`当前 grapheme 使用当前解析色、清晰打印印章使用 `stamp`。关闭、无有效缓存或请求失败时完整保留默认配色；不新增 ticker。
