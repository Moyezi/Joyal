...
- `浮名` 的当前字打印印章保持清晰硬边，不使用 `MaskFilter.blur`；关键词印章使用关键词语义色，其他印章使用配置的 `stamp` 色，峰值 alpha 为解析后印章颜色 alpha 的 0.8 倍，并随印章脉冲淡出。只有当前句完成到下一条非空同步歌词的完整间隔达到 3.6 秒时，才从句尾完成时立即以输入等待光标的节奏继续显示小型硬边印章，不得等待 3.6 秒后才出现；下一句前提前收住，并复用现有 frame controller。最后一句歌词不应用 3.6 秒门槛，打印完成后同样立即显示等待光标直到播放停止。句尾最后一个有效文字单元属于关键词时，等待光标继承该关键词的语义色，末尾标点不阻断继承；暂停、舞台隐藏/遮挡、关闭逐字或 reduced motion 时不显示。空白 grapheme 和所有 Unicode 标点符号不绘制印章。默认滚动歌词仍可保留自身的模糊打印印章。已唱/未唱字形必须通过逐 grapheme 的 TextSpan 颜色区分，不能再用 glyph 选区矩形裁剪整段亮色 TextPainter；紧行距或字体 ink bounds 溢出时，后者会误揭示下一视觉行的字顶。当前句切成已唱句时，复用镜头转场把非关键词字从当前色平滑过渡到已唱灰色，关键词色保持不变。
...
- 重新获取当前歌曲的 AI 配色时，先备份现有配色，再通过 `LyricsAiPaletteRepository.delete()` 同时删除 current/legacy 缓存，然后 invalidate 并等待普通 `lyricsAiPaletteProvider(LyricsAiPaletteRequest(song, lyrics))`；不要再引入独立的 `forceRefresh` family identity。生成失败时恢复备份并 invalidate 普通 provider，成功时刷新 `recognizedLyricsAiPalettesProvider`。
- `lyricsAiPaletteProvider` 是 `autoDispose` 异步 provider；在缓存读取和 DeepSeek 请求完成前用 `ref.keepAlive()` 保活，并在 `finally` 中关闭 link，避免页面重建或无监听的 controller `.future` 等待被回收后误报生成失败。回归测试必须让假配色服务延迟返回且不依赖 UI listener。
